...
#GillKieren_Assign9_problem2_help.py
...
for character in secret:
    if character == 'S':
        x=secret[(secret.index('S'))+1]
        y=secret[(secret.index('S'))+2]
        
        turtle.penup()
        turtle.goto(x,y)
        
        secret.remove(character)
        secret.remove(x)
        secret.remove(y)


    elif character == 'P':
        x=secret[(secret.index('P'))+1]
        y=secret[(secret.index('P'))+2]
        ...
        x=secret[(secret.index('E'))+1]
        y=secret[(secret.index('E'))+2]
